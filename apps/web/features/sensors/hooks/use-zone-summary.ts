import { SensorStatus, SensorZone, zoneForRoutingKey } from "@repo/shared-types";
...
const ZONE_ORDER: SensorZone[] = [SensorZone.HVAC, SensorZone.FLOOR];

// Groups live sensors by zone and aggregates the worst status per zone (drives
// the zone dots + node counts). Memoised against the `sensors` record reference.
