  ...
  return (
    <div className="glass-panel pointer-events-auto flex h-96 w-full flex-col rounded-xl p-5 md:h-auto md:max-h-[calc(100vh-3rem)] md:w-80">
      <div className="mb-4 flex items-center justify-between">
        ...
        <span className="rounded border border-slate-200 bg-slate-100 px-2 py-1 font-mono text-[10px] text-slate-500">
          LIVE
        </span>
      </div>
      ...
      <div className="flex flex-grow flex-col gap-3 overflow-y-auto pr-2">
        {alerts.length === 0 ? (
          <p className="mt-10 text-center text-xs text-slate-500">Waiting for events…</p>
        ) : (
...
function AlertRow({ alert }: { alert: AlertEvent }) {
  ...
  return (
    <button
      type="button"
      onClick={handleClick}
      className="flex cursor-pointer flex-col gap-1 rounded border p-3 text-left transition-all hover:shadow-md"
      style={{
